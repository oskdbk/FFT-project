...
*/
void contour_to_complex_vector_helper(const vector<cv::Point>& contour, vector<complex_num>& result, size_t start, size_t end) {
    for (size_t i = start; i < end; i++) {
        auto point = contour[i];
        result[i] = complex_num(point.x, point.y);
    }
}

vector<complex_num> contour_to_complex_vector(const vector<cv::Point>& contour, size_t num_threads = 8) {
    ...
    for (size_t i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back([&contour, &result, start, end]() {
            contour_to_complex_vector_helper(contour, result, start, end);
        });
    }
...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    vector<complex_num> contour_complex = contour_to_complex_vector(contour, num_threads);
    ...
    for (float t = 0; t < 2*M_PI; t += 4*M_PI/fft.size()) {
        auto pointt = parametric(complex_num(WIDTH/2.0 + 100, 100), 0, t, fft, num_threads);
